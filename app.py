...
def get_gradcam(model, image: Image.Image, pred_idx: int, arch: str):
    """Returns a heatmap PIL image overlaid on the original scan."""
    ...
    if arch == "convnext":
        ...
        def reshape_transform(tensor, height=14, width=14):
            # ViT outputs [batch, 197, channels] — reshape to spatial map
    ...
    try:
        with GradCAM(
            ...
            reshape_transform = reshape_transform if arch == "vit" else None,
        ) as cam:
            grayscale_cam = cam(
                input_tensor = img_tensor,
                targets      = targets,
                aug_smooth   = True,    # smooths the heatmap
                eigen_smooth = True,    # reduces noise
            )[0]
        ...
        heatmap_img = show_cam_on_image(
            ...
            grayscale_cam,
            use_rgb    = True,
            colormap   = 17,  # cv2.COLORMAP_JET — red=high attention
            image_weight = 0.55,
        )
...
with col_results:
    ...
    if not uploaded:
        ...
        st.info("⚠️ This result is generated by an AI model and must not be used for clinical diagnosis without review by a qualified medical professional.")
        ...
        with st.expander("🔍 Explainability — Grad-CAM heatmap", expanded=True):
            st.markdown("""
            <p style="font-size:0.82rem;color:#797876;margin-bottom:0.75rem;">
            ...
            with st.spinner("Generating heatmap…"):
                heatmap = get_gradcam(
                    model    = model,
                    image    = image,
                    pred_idx = cfg["classes"].index(pred_cls),
                    arch     = cfg["model_arch"],
                )
            ...
            if heatmap:
                ...
                with col_heat:
                    st.image(heatmap,
                            caption=f"Grad-CAM — {cls_name}",
                            use_container_width=True)
            else:
                st.info("Heatmap unavailable for this model.")
